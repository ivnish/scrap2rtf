...
void usage (int status)
{
	...
	else
	{
        ...
        cout << _("Report "PROGRAM_NAME" bugs to ivnitsky.a@gmail.com") << endl;
        cout << _(PROGRAM_NAME" home page: <http://code.google.com/p/scrap2rtf/>") << endl;
...
int scrapAnalize(const string szFileName)
{
    ...
    {
        if (bVerbose)
            cout << _(PROGRAM_NAME": File ") << szFileName << _(" was create is MS Word 2003/2007/2010") << endl;
        return DOC_DOCX;
    }
    else
    /* Else if Workbook exists, then it is Excel 2003  */
    if (FileExists(szTempDir+"/[3]ITEM000/Workbook"))
    {
        if (bVerbose)
            cout << _(PROGRAM_NAME": File ") << szFileName << _(" was create is MS Excel 2003") << endl;
        return XLS;
    }
    else
    /* Else if Package exists, then it is Excel 2007/2010  */
    if (FileExists(szTempDir+"/[3]ITEM000/Package"))
    {
        if (bVerbose)
            cout << _(PROGRAM_NAME": File ") << szFileName << _(" was create is MS Excel 2007/2010") << endl;
        return XLSX;
    }
    else /* Sorry, it is not scrap */
    {
...
{
    /* Form a path for the future RTF */
    ...
    if (bVerbose)
        cout << PROGRAM_NAME": " << _("File ") << szFileName << _(" successfully converted to file ") << szRTFfileName << endl;
...
{
    /* Form a path for the future XLS */
    ...
    if (bVerbose)
        cout << PROGRAM_NAME": " << _("File ") << szFileName << _(" successfully converted to file ") << szXLSfileName << endl;
...
{
    /* Form a path for the future XLSX */
    ...
    if (bVerbose)
        cout << PROGRAM_NAME": " << _("File ") << szFileName << _(" successfully converted to file ") << szXLSXfileName << endl;
...
void scrapConvert(const string szFileName, int nScrapFormat)
{
    switch (nScrapFormat)
    {
        case DOC_DOCX: Convert2RTF(szFileName); break;
        case XLS: Convert2XLS(szFileName); break;
        case XLSX: Convert2XLSX(szFileName); break;
    }
...
void processFiles(const string szFileName)
{
	...
	if (nScrapFormat != NOTSCRAP)
	{
		...
		if (bRemOrig)
		{
			...
			if (bVerbose)
				cout << PROGRAM_NAME": " << _("File ") << szFileName << _(" successfully removed") << endl;
		}
	}
}
//========================================================================
...
int main (int argc, char **argv)
{
    ...
    while ((optc = getopt_long (argc, argv, short_options, long_options, NULL)) != -1)
    {
        switch (optc)
        {
            ...
            case 'r': bRemOrig = 1; break;
            case 'o': szOutputDir = optarg; break;
            case 's': bSameDir = 1; break;
            case GETOPT_HELP_CHAR:   usage (EXIT_SUCCESS); break;
            case GETOPT_VERSION_CHAR:
            cout << PROGRAM_NAME << " " << VERSION << endl;
            cout << "Copyright (C) 2009-2014 "AUTHORS << endl;
            ...
            cout << _("Written by ") << AUTHORS << endl; exit(EXIT_SUCCESS); break;
